...
void MPU9250::read_acc()
{
    ...
    for(i=0; i<3; i++) {
        ...
        data=(float)bit_data;
        accelerometer_data[i]=data/acc_divider - accelBias[i];
    }
...
void MPU9250::read_rot()
{
    ...
    for(i=0; i<3; i++) {
        bit_data=((int16_t)response[i*2]<<8)|response[i*2+1];
        data=(float)bit_data;
        gyroscope_data[i]=data/gyro_divider - gyroBias[i];
    }
...
void MPU9250::read_all(){
    ...
    for(i=4; i<7; i++) {
        bit_data=((int16_t)response[i*2]<<8)|response[i*2+1];
        data=(float)bit_data;
        gyroscope_data[i-4]=data/gyro_divider - gyroBias[i-4];
    }
...
void MPU9250::select() {
    ...
    SPI.beginTransaction(SPISettings(myClock, MSBFIRST, SPI_MODE3));
    digitalWriteFast(mycs, LOW);
}
